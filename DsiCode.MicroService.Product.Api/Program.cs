using AutoMapper;
using DsiCode.MicroService.Product.API;
using DsiCode.MicroService.Product.API.Data;
using DsiCode.MicroService.Product.API.Extensions;
using DsiCode.MicroService.Product.API.Services;
using DsiCode.MicroService.Product.API.Services.IServices;
...
// ✅ Database configuration - MySQL con esquemas separados
...
builder.Services.AddSwaggerGen(option =>
{
    option.SwaggerDoc("v1", new OpenApiInfo
    {
        Title = "Product API",
        Version = "v1",
        Description = "API de productos para microservicios"
    });
...
// ✅ IMPORTANTE: Crear directorio de imágenes antes de configurar archivos estáticos
var imagesPath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "images");
if (!Directory.Exists(imagesPath))
{
    Directory.CreateDirectory(imagesPath);
    Console.WriteLine($"✅ Directorio de imágenes creado: {imagesPath}");
}
...
using (var scope = app.Services.CreateScope())
{
    ...
    for (int i = 0; i < maxRetries; i++)
    {
        try
        {
            Console.WriteLine($"🔄 ProductDB: Intento {i + 1} - Aplicando migraciones...");
            ...
            Console.WriteLine("✅ ProductDB: Migraciones aplicadas correctamente");
            break; // ✅ Salir del loop si es exitoso
        }
        catch (Exception ex)
        {
            Console.WriteLine($"❌ ProductDB: Error en intento {i + 1}: {ex.Message}");

            if (i == maxRetries - 1)
            {
                Console.WriteLine($"❌ ProductDB: Falló después de {maxRetries} intentos");
                throw; // ✅ Re-lanzar excepción en el último intento
            }

            Console.WriteLine($"⏳ ProductDB: Esperando {delay.TotalSeconds}s antes del siguiente intento...");
            await Task.Delay(delay);
        }
    }
}

Console.WriteLine("🚀 Microservicio de Productos iniciado correctamente");
app.Run();